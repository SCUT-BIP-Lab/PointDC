...
parser.add_argument('--input_path', type=str, default='/home/bip/czs/codes/growsp_inf/data/S3DIS4growsp/input_rebuild', help='raw data path')
parser.add_argument('--sp_path', type=str, default='/home/bip/czs/codes/growsp_inf/data/S3DIS4growsp/initial_superpoints_rebuild')
...
def construct_superpoints(path):
    ...
    coords, colors, labels, unique_map, inverse_map = ME.utils.sparse_quantize(np.ascontiguousarray(coords),
                            colors, labels=labels, ignore_label=-1, return_index=True, return_inverse=True)
    coords = coords.numpy().astype(np.float32)

    '''VCCS'''
    out = supervoxel_clustering(coords, colors)
    ...
    out_coords = pcddict['coord'].copy()
    out_labels = np.zeros(pcddict['coord'].shape[0])
    #
    if not exists(args.sp_path):
        os.makedirs(args.sp_path)
    np.save(args.sp_path + '/' + filename[:-3] + '_superpoint.npy', out_sp_labels)
    ...
    if vis:
        ...
        write_ply(vis_path + '/' + filename.replace('.pt', '.ply'), [out_coords, colors], ['x', 'y', 'z', 'red', 'green', 'blue'])
    ...
    print('completed scene: {}, used time: {:.2f}s'.format(filename, time.time() - time_start))
    return (out_labels, sp2gt)

print('start constructing initial superpoints')
path_list = []
folders = sorted(glob.glob(args.input_path + '/*.pt'))
